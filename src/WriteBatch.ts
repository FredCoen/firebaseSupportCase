...
import { DocumentReference, DocumentData, SetOptions, UpdateData, Precondition, FieldPath, WriteResult, Timestamp } from "@google-cloud/firestore";
...
export class WriteBatch {
    ...
    // we make a proprty read only so it cannot be modified
    ...
    // we do return the instace of the WriteBatch in order to allow the chaining of calls, example: batch.set(....).update(..)
    ...
    update(documentRef: DocumentReference, dataOrField: UpdateData | string | FieldPath, ...preconditionOrValues: any[]): this {
    ...
    commit = async () => ([] as WriteResult[]).concat(...await Promise.all(this.batches.map(b => b.commit())))
